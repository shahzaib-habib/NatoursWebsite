...
var hideAlert = function hideAlert() {
  var el = document.querySelector('.alert'); // Js trick
  // where we need to move one level up to the parent element and then from there 
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              location.reload(true); // 'true' and that will then force a reload from the server and not from 
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            console.log(session); // now we need to connect the green button to the function that we just created inside stripe.js
...
/* eslint-disable */
// if eslint is enabled and especially for node.js then disable it for this file only
// DOM ELEMENTS
...
if (mapBox) {
  var locations = JSON.parse(mapBox.dataset.locations); // probably at the time we called "getElementById" the DOM is not actually loaded
  // because we have our scripts integrated right at the beginning of the "tour tamplate"

  (0, _mapbox.displayMap)(locations);
}

if (loginForm) {
  // we use "querySelector" for selecting using className
  loginForm.addEventListener('submit', function (e) {
    // this prevents form from loading any other page
    e.preventDefault();
    var email = document.getElementById('email').value;
    var password = document.getElementById('password').value; // lets do the actual login in a separate function

    (0, _login.login)(email, password); // we also need to include this login file into our base template
  });
}
